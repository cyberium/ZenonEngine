...
QueryDX11::QueryDX11(IRenderDeviceDX11& RenderDeviceDX11, QueryType queryType, uint8_t numBuffers)
...
void QueryDX11::Begin(int64_t frame)
{
	...
	if (buffer >= 0)
	{
		...
		{
			m_RenderDeviceDX11.GetDeviceContextD3D11()->Begin(m_DisjointQueries[buffer]);
			m_RenderDeviceDX11.GetDeviceContextD3D11()->End(m_Queries[0][buffer]);
		}
		else
		{
			m_RenderDeviceDX11.GetDeviceContextD3D11()->Begin(m_Queries[0][buffer]);
		}
	}
}
...
void QueryDX11::End(int64_t frame)
{
	...
	if (buffer >= 0)
	{
		if (m_QueryType == QueryType::Timer)
		{
			m_RenderDeviceDX11.GetDeviceContextD3D11()->End(m_Queries[1][buffer]);
			m_RenderDeviceDX11.GetDeviceContextD3D11()->End(m_DisjointQueries[buffer]);
		}
		else
		{
			m_RenderDeviceDX11.GetDeviceContextD3D11()->End(m_Queries[0][buffer]);
...
bool QueryDX11::QueryResultAvailable(int64_t frame)
{
	...
	if (buffer >= 0)
	{
		if (m_QueryType == QueryType::Timer)
		{
			result = (m_RenderDeviceDX11.GetDeviceContextD3D11()->GetData(m_DisjointQueries[buffer], nullptr, 0, 0) == S_OK);
		}
		else
		{
			result = (m_RenderDeviceDX11.GetDeviceContextD3D11()->GetData(m_Queries[0][buffer], nullptr, 0, 0) == S_OK);
		}
	}
...
IQuery::QueryResult QueryDX11::GetQueryResult(int64_t frame)
{
	...
	if (buffer >= 0)
	{
		if (m_QueryType == QueryType::Timer)
		{
			while (m_RenderDeviceDX11.GetDeviceContextD3D11()->GetData(m_DisjointQueries[buffer], nullptr, 0, 0) == S_FALSE)
			{
				Sleep(1L);
			}
			D3D11_QUERY_DATA_TIMESTAMP_DISJOINT timeStampDisjoint;
			m_RenderDeviceDX11.GetDeviceContextD3D11()->GetData(m_DisjointQueries[buffer], &timeStampDisjoint, sizeof(D3D11_QUERY_DATA_TIMESTAMP_DISJOINT), 0);
			if (timeStampDisjoint.Disjoint == FALSE)
			{
				UINT64 beginTime, endTime;
				if (m_RenderDeviceDX11.GetDeviceContextD3D11()->GetData(m_Queries[0][buffer], &beginTime, sizeof(UINT64), 0) == S_OK &&
					m_RenderDeviceDX11.GetDeviceContextD3D11()->GetData(m_Queries[1][buffer], &endTime, sizeof(UINT64), 0) == S_OK)
				{
		...
		else
		{
			...
			while (m_RenderDeviceDX11.GetDeviceContextD3D11()->GetData(m_Queries[0][buffer], nullptr, 0, 0))
			{
				Sleep(1L);
			}
			...
			{
				UINT64 numSamples = 0;
				if (m_RenderDeviceDX11.GetDeviceContextD3D11()->GetData(m_Queries[0][buffer], &numSamples, sizeof(UINT64), 0) == S_OK)
				{
			...
			case IQuery::QueryType::CountSamplesPredicate:
			{
				...
				if (m_RenderDeviceDX11.GetDeviceContextD3D11()->GetData(m_Queries[0][buffer], &anySamples, sizeof(UINT64), 0) == S_OK)
				{
			...
			case IQuery::QueryType::CountTransformFeedbackPrimitives:
			{
				...
				if (m_RenderDeviceDX11.GetDeviceContextD3D11()->GetData(m_Queries[0][buffer], &streamOutStats, sizeof(D3D11_QUERY_DATA_SO_STATISTICS), 0) == S_OK)
				{
