...
// General
#include "ApplicationNative_PlatformWindows.h"
...
CApplicationNative_PlatformWindows::CApplicationNative_PlatformWindows()
{
	...
	InitializeEngineInternal({ "znRenderDX11.dll" , "znPluginFBXModels.dll" });
}
...
// IApplication
//
std::unique_ptr<IznNativeWindow> CApplicationNative_PlatformWindows::CreateNativeWindow(const std::string & WindowName, glm::ivec2 WindowSize) const
{
...
std::unique_ptr<IznNativeWindow> CApplicationNative_PlatformWindows::CreateNativeWindowProxy(IznNativeWindow & SourceNativeWindow) const
{
...
//
void CApplicationNative_PlatformWindows::ShowMessageBox(const std::string & Title, const std::string & Message) const
{
	MessageBoxA(HWND_DESKTOP, Message.c_str(), Title.c_str(), MB_ICONERROR | MB_OK);
}

std::string CApplicationNative_PlatformWindows::GetExecutablePath() const
{
...
IznPlugin* CApplicationNative_PlatformWindows::LoadPlugin(const std::string & PluginFilename) const
{
	try
	{
...
void CApplicationNative_PlatformWindows::UnloadPlugin(const std::string& PluginFilename) const
{
...
//
HINSTANCE CApplicationNative_PlatformWindows::GetHInstance() const
{
	return m_HInstance;
}
...
// Protected
//
int CApplicationNative_PlatformWindows::ProcessEvents()
{
	MSG msg = { 0 };
...
void CApplicationNative_PlatformWindows::ExitApplication()
{
	PostQuitMessage(0);
}
