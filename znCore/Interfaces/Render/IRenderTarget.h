...
ZN_INTERFACE OW_ENGINE_API IRenderTarget
{
	enum class OW_ENGINE_API AttachmentPoint
	{
	...
	virtual std::shared_ptr<ITexture> GetTexture(AttachmentPoint attachment) = 0;
	...
	*/
	...
	virtual std::shared_ptr<IStructuredBuffer> GetStructuredBuffer(uint8_t slot) = 0;
