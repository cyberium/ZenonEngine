...
// CBaseList3DPass
//
CBaseList3DPass::CBaseList3DPass(IRenderDevice& RenderDevice, const std::shared_ptr<CSceneNodeListPass>& SceneNodeListPass, SceneNodeType SceneNodeType)
...
CBaseList3DPass::CBaseList3DPass(IRenderDevice & RenderDevice, const std::shared_ptr<CSceneNodeListPass>& SceneNodeListPass, std::vector<SceneNodeType> SceneNodeTypesList)
	: RenderPassPipelined(RenderDevice)
	, m_SceneNodeListPass(SceneNodeListPass)
	, m_PerObjectParameter(nullptr)
{
...
void CBaseList3DPass::Render(RenderEventArgs & e)
{
	std::for_each(m_AcceptSceneNodeTypes.begin(), m_AcceptSceneNodeTypes.end(), [this](SceneNodeType SceneNodeType) {
		...
		std::for_each(list.begin(), list.end(), [this](const ISceneNode3D* SceneNode) {
			if (Visit(SceneNode))
			{
				const auto& components = SceneNode->GetComponents();
