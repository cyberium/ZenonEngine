...
ZN_INTERFACE ZN_API IBlendState
{
	virtual ~IBlendState() {}
	 ...
	 */
	enum class ZN_API BlendOperation // could also be called BlendOperand
	{
	...
	enum class ZN_API BlendFactor
	{
		...
		ConstBlendFactor,   // Multiply the pixel operand by a constant blend factor before applying the blend operation. See @IBlendState::SetConstBlendFactor.
		OneMinusBlendFactor,// Multiply the pixel operand by ( 1 - blend factor ) before applying the blend operation. See @IBlendState::SetBlendFactor.
	 ...
	 */
	enum class ZN_API LogicOperator
	{
