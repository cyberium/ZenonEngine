...
// General
#include "PropertyDelegateString.h"
...
{
	factory.registerDelegateDefault(&QtnPropertyStringBase::staticMetaObject
		, &qtnCreateDelegate<QtnPropertyDelegateQString, QtnPropertyStringBase>
		, "LineEdit");

	factory.registerDelegate(&QtnPropertyStringBase::staticMetaObject
		, &qtnCreateDelegate<QtnPropertyDelegateQStringFile, QtnPropertyStringBase>
		, "File");

	factory.registerDelegate(&QtnPropertyStringBase::staticMetaObject
		, &qtnCreateDelegate<QtnPropertyDelegateQStringList, QtnPropertyStringBase>
		, "List");

	factory.registerDelegate(&QtnPropertyStringBase::staticMetaObject
		, &qtnCreateDelegate<QtnPropertyDelegateQStringCallback, QtnPropertyStringBase>
		, "Callback");
}

class QtnPropertyQStringLineEditHandler : public QtnPropertyEditorHandler<QtnPropertyStringBase, QLineEdit>
{
public:
	QtnPropertyQStringLineEditHandler(QtnPropertyStringBase& property, QLineEdit& editor)
...
private:
	...
	{
		editor().setText(property().value().c_str());
	}

	void onEditingFinished()
	{
		property().setValue(editor().text().toStdString());
	}
...
QtnPropertyDelegateQString::QtnPropertyDelegateQString(QtnPropertyStringBase& owner)
	: QtnPropertyDelegateTyped<QtnPropertyStringBase>(owner)
{
}

bool QtnPropertyDelegateQString::acceptKeyPressedForInplaceEditImpl(QKeyEvent *keyEvent) const
{
	if (QtnPropertyDelegateTyped<QtnPropertyStringBase>::acceptKeyPressedForInplaceEditImpl(keyEvent))
		return true;
...
{
	strValue = owner().value().c_str();
	return true;
}

class QtnPropertyQStringFileLineEditBttnHandler : public QtnPropertyEditorHandler<QtnPropertyStringBase, QtnLineEditBttn>
{
public:
	QtnPropertyQStringFileLineEditBttnHandler(QtnPropertyStringBase& property, QtnLineEditBttn& editor)
		: QtnPropertyEditorHandlerType(property, editor),
		m_dlg(&editor)
	{
...
private:
	void updateEditor() override
	{
		editor().lineEdit->setText(property().value().c_str());
	}
	...
	void onToolButtonClicked(bool checked)
	{
		...
		m_dlg.selectFile(property().value().c_str());
		if (m_dlg.exec() == QDialog::Accepted)
		{
			QStringList files = m_dlg.selectedFiles();
			if (files.size() == 1)
				property().setValue(files.first().toStdString());
		}
	}

	void onEditingFinished()
	{
		property().setValue(editor().lineEdit->text().toStdString());
	}
...
QtnPropertyDelegateQStringInvalidBase::QtnPropertyDelegateQStringInvalidBase(QtnPropertyStringBase& owner)
...
QtnPropertyDelegateQStringFile::QtnPropertyDelegateQStringFile(QtnPropertyStringBase& owner)
...
{
	return QFileInfo(owner().value().c_str()).exists();
}

class QtnPropertyQStringListComboBoxHandler : public QtnPropertyEditorHandler<QtnPropertyStringBase, QComboBox>
{
public:
	QtnPropertyQStringListComboBoxHandler(QtnPropertyStringBase& property, QComboBox& editor)
		: QtnPropertyEditorHandlerType(property, editor)
	{
		updateEditor();
...
private:
	void updateEditor() override
	{
		editor().setCurrentText(property().value().c_str());
	}

	void onCurrentTextChanged(const QString& text)
	{
		property().setValue(text.toStdString());
	}
};

QtnPropertyDelegateQStringList::QtnPropertyDelegateQStringList(QtnPropertyStringBase& owner)
	: QtnPropertyDelegateQString(owner)
{
}
...
QWidget* QtnPropertyDelegateQStringList::createValueEditorImpl(QWidget* parent, const QRect& rect, QtnInplaceInfo* inplaceInfo)
{
	if (!owner().isEditableByUser())
	{
		...
		lineEdit->setText(owner().value().c_str());
...
/*
class QtnPropertyQStringCallbackLineEditBttnHandler: public QtnPropertyEditorHandler<QtnPropertyStringBase, QtnLineEditBttn>
{
public:
	QtnPropertyQStringCallbackLineEditBttnHandler(QtnPropertyStringBase& property, QtnLineEditBttn& editor, const QtnPropertyDelegateAttributes& attributes)
		: QtnPropertyEditorHandlerType(property, editor)
	{
...
class QtnPropertyQStringCandidatesComboBoxHandler : public QtnPropertyEditorHandler<QtnPropertyStringBase, QtnComboBoxBttn>
{
public:
	QtnPropertyQStringCandidatesComboBoxHandler(QtnPropertyStringBase& property, QtnComboBoxBttn& editor, const QtnPropertyDelegateAttributes& attributes)
		: QtnPropertyEditorHandlerType(property, editor)
	{
...
private:
	...
	void updateEditor() override
	{
		...
		editor().comboBox->setCurrentText(property().value().c_str());
	}
	...
	void createNewCandidate(QString candidate)
	{
		...
		property().setValue(candidate.toStdString());
		updateCandidates();
...
QtnPropertyDelegateQStringCallback::QtnPropertyDelegateQStringCallback(QtnPropertyStringBase& owner)
	: QtnPropertyDelegateQString(owner)
{
}
